...
@app.route("/_create")
def new_memo():
  app.logger.debug("Got a JSON request: CREATE")
...
def format_arrow_date( date ):
    try: 
        ...
        normal = arrow.get(date) # lots of formats in case the person is an average user
        app.logger.debug(normal)
        return normal.to('local').format("YYYY-MM-DD")
    except:
        return "(baaad date)"
...
#         return normal.to('local').format("MM/DD/YYYY")
#     except:
#         return "(bad date)"
